...
export default function Dashboard() {
  ...
  const handleLogout = () => {
    ...
    sessionStorage.removeItem("userRole"); // Remove user role
    router.push("/login"); // Redirect to login page
  };

  // Sample data for the graph
  const data = {
    labels: userStats.map((stat) => stat.month), // Example: ["Jan", "Feb", "Mar"]
    datasets: [
      {
        label: "New Users",
        data: userStats.map((stat) => stat.count), // Example: [50, 75, 120]
